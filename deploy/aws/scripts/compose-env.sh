...
if [[ "${ENABLE_OPENWA:-0}" == "1" ]]; then
  ...
  : "${OPENWA_DEDICATED_NUMBER_REF:?OPENWA_DEDICATED_NUMBER_REF is required when ENABLE_OPENWA=1}"
  ...
  if [[ "${OPENWA_COMMERCIAL_USE:-0}" == "1" ]]; then
    if [[ "${OPENWA_COMMERCIAL_GATE_APPROVED:-0}" != "1" ]]; then
      echo 'Refusing OpenWA commercial use: commercial gate is not approved.' >&2
      exit 2
    fi
    : "${OPENWA_GATE_EVIDENCE_FILE:?OPENWA_GATE_EVIDENCE_FILE is required for commercial use}"
    if [[ ! -f "$OPENWA_GATE_EVIDENCE_FILE" ]]; then
      echo 'Refusing OpenWA commercial use: gate evidence file is missing.' >&2
      exit 2
    fi
    if [[ ! "${OPENWA_GATE_EVIDENCE_SHA256:-}" =~ ^[0-9a-fA-F]{64}$ ]]; then
      echo 'Refusing OpenWA commercial use: gate evidence SHA-256 is required.' >&2
      exit 2
    fi
    evidence_sha256="$(sha256sum "$OPENWA_GATE_EVIDENCE_FILE" | awk '{print $1}')"
    if [[ "${evidence_sha256,,}" != "${OPENWA_GATE_EVIDENCE_SHA256,,}" ]]; then
      echo 'Refusing OpenWA commercial use: gate evidence hash mismatch.' >&2
      exit 2
    fi
    if ! python3 "$REPO_ROOT/scripts/validate_openwa_gate.py" "$OPENWA_GATE_EVIDENCE_FILE" > /dev/null; then
      echo 'Refusing OpenWA commercial use: gate manifest is not approved.' >&2
      exit 2
    fi
  fi
