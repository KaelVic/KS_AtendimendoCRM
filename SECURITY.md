...
O KS Atendimento IA é distribuído em rolling release a partir da branch `main`. Correções de segurança são aplicadas apenas à versão mais recente — mantenha sua instalação atualizada (`bash hostgator-setup-kit/update.sh` em self-host).
...
Use o [relato privado de vulnerabilidades do GitHub](https://github.com/KaelVic/KS_AtendimendoCRM/security/advisories/new) — o relato chega só aos mantenedores, e o histórico fica auditável.
