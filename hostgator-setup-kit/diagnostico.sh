...
#
#   curl -fsSL https://raw.githubusercontent.com/KaelVic/KS_AtendimendoCRM/main/hostgator-setup-kit/diagnostico.sh | bash
#
...
# O install.sh clona em ./ks_atendimentocrm por padrão, mas o operador pode ter posto
...
achar_projeto() {
  local c
  for c in . ./ks_atendimentocrm /root/KS Atendimento IA /root/ks_atendimentocrm /opt/ks_atendimentocrm /var/www/crm; do
...
PROJETO="$(achar_projeto)" || {
  printf '%s✖ Não achei uma instalação do KS Atendimento IA nesta máquina.%s\n' "$R" "$Z"
  printf '  Rode este script de dentro da pasta do projeto (a que tem %s).\n' "$COMPOSE_FILE"
  exit 2
}
cd "$PROJETO" || exit 2

printf '%s══ Diagnóstico do KS Atendimento IA ══%s\n' "$B" "$Z"
...
#     como `<namespace>/ks_atendimentocrm-worker:falsificado`, e o diagnóstico
