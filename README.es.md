...
# 🛠️ KS Atendimento IA — el Sistema Operativo de Ventas con IA, open source, para WhatsApp
...
>
> El [`hostgator-setup-kit/`](hostgator-setup-kit/) instala el CRM completo (app + WhatsApp + base de datos) en un VPS con un único comando, y el [runbook de producción](docs/runbooks/waha-hostgator.md) ya asume ese entorno.
>
> Recomendamos un **VPS con Docker y al menos 4 GB de RAM** (HostGator, Hostinger, Hetzner o tu proveedor de preferencia).
>
> **¿Todavía no tienes servidor?** Ejecuta esto **en tu propia computadora** (macOS, Linux o WSL). Te devuelve el comando exacto para tu caso:
>
> ```bash
> curl -fsSL https://raw.githubusercontent.com/KaelVic/KS_AtendimendoCRM/main/hostgator-setup-kit/comecar.sh | bash
> ```
>
> *(¿prefieres leer antes de ejecutar? clona el repo y corre `bash hostgator-setup-kit/comecar.sh` — no instala nada sin que confirmes.)*
...
|---|---|
| **VPS con Docker** | HostGator, Hostinger, Hetzner o cualquier VPS con Docker. 4 GB de RAM recomendados |
...
y dile *"instálame el KS Atendimento IA"*. Lee el [`CLAUDE.md`](hostgator-setup-kit/CLAUDE.md) del kit
...
> `update.sh` **dos vezes**: la primera es todavía el script viejo (que baja el nuevo); la
...
**KS Atendimento IA** es la plataforma de KaelSolutions para centralizar CRM, atención por WhatsApp y agentes de inteligencia artificial en una sola operación.

El proyecto corre en **clínicas, inmobiliarias, infoproductos, agencias, tiendas y prestadores de servicios** — cualquier negocio que venda por WhatsApp. Es un **sistema operativo de ventas**: agentes de IA con RAG por tenant atienden, califican, mueven leads en el embudo, disparan automatizaciones y saben cuándo pasarle la conversación a una persona — con todo el CRM expuesto vía **MCP** para que los agentes lo operen de verdad. La historia completa está en [`VISION.md`](VISION.md).
...
| **Hosting** | Cualquier VPS con Docker (HostGator, Hostinger, Hetzner, etc.) | App + WhatsApp + workers en tu propia máquina |
...
- **Docker y VPS** — la base de infraestructura que hizo posible el self-host de 1 comando.
