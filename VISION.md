...
Toda a operação comercial de um negócio — atendimento, qualificação, funil, pós-venda — operada em um único lugar, por pessoas e por agentes de IA trabalhando juntos.

O "CRM" é a categoria de entrada, não o teto. O KS Atendimento IA é **mais que um CRM**: é o sistema onde a venda acontece.

## De onde viemos, pra onde vamos

O projeto nasceu em 2026 como um CRM operacional para **e-commerce brasileiro** — WhatsApp via WAHA, integração Nuvemshop, LGPD nativa. Quando abrimos o código, a comunidade decidiu outra coisa: a maioria dos adopters passou a rodar o sistema em **clínicas, infoprodutos, imobiliárias, agências e prestadores de serviço** — qualquer negócio que vende conversando.
...
3. **MCP como sistema nervoso.** O CRM inteiro é exposto como tools MCP — primeiro para os agentes internos, depois como contrato público. Um negócio deve poder plugar o agente que quiser (Claude, o que vier) e ele **opera** o KS Atendimento IA: cria lead, responde cliente, agenda, consulta pedido. O CRM vira infraestrutura para agentes.
...
**Uma frase (pt-br):**
> KS Atendimento IA é o sistema operacional de vendas open source com agentes de IA nativos e WhatsApp — self-hosted, multi-tenant, para qualquer negócio que vende conversando.

**One-liner (en):**
> KS Atendimento IA is an open-source AI sales OS: a self-hosted CRM where AI agents natively operate sales and support over WhatsApp — an open alternative to Kommo, Octadesk and Intercom.
...
- **A infraestrutura é self-hosted:** o projeto é distribuído como código aberto (MIT). O caminho recomendado para produção é uma VPS com Docker (HostGator, Hostinger, Hetzner, etc.), instalada pelo `hostgator-setup-kit` com 1 comando.
- **O caminho genérico nunca é sabotado:** `docker compose` e o kit self-host funcionam em qualquer VPS com Docker.
...
5. **Transparência de modelo.** Requisitos de infraestrutura e telemetria declarados em linguagem humana no README, nunca escondidos.
