...
cat > "$PROJ/.env" <<ENV
APP_IMAGE=${NS}/ks_atendimentocrm:latest
...
check ".env aponta para a imagem da versão instalada" grep -q "^APP_IMAGE=${NS}/ks_atendimentocrm:1.1.0$" .env
...
check "o worker é pinado na MESMA versão do app" \
  grep -q "^WORKER_IMAGE=${NS}/ks_atendimentocrm-worker:1.1.0$" .env
check "o scheduler é pinado na MESMA versão do app" \
  grep -q "^SCHEDULER_IMAGE=${NS}/ks_atendimentocrm-scheduler:1.1.0$" .env
...
check "a imagem do .env continua intacta" grep -q "^APP_IMAGE=${NS}/ks_atendimentocrm:latest$" .env
...
check "NÃO reescreveu a imagem do .env" grep -q "^APP_IMAGE=${NS}/ks_atendimentocrm:latest$" .env
...
pin_caso "app pinado + worker/scheduler AUSENTES → acusa os dois" \
  "APP_IMAGE=${NS}/ks_atendimentocrm:1.3.0" "worker scheduler"
pin_caso "app pinado + worker em canal móvel → acusa" \
  "APP_IMAGE=${NS}/ks_atendimentocrm:1.3.0
WORKER_IMAGE=${NS}/ks_atendimentocrm-worker:stable
SCHEDULER_IMAGE=${NS}/ks_atendimentocrm-scheduler:1.3.0" "worker"
pin_caso "as três na mesma versão → silêncio" \
  "APP_IMAGE=${NS}/ks_atendimentocrm:1.3.0
WORKER_IMAGE=${NS}/ks_atendimentocrm-worker:1.3.0
SCHEDULER_IMAGE=${NS}/ks_atendimentocrm-scheduler:1.3.0" ""
...
case "$*" in
  ...
  *"Config.Image"*)  printf '%s/ks_atendimentocrm-worker:stable\n' "${NS:?dublê de docker sem NS no ambiente}" ;;
...
R="$(autopin "APP_IMAGE=${NS}/ks_atendimentocrm:1.3.0
WORKER_IMAGE=${NS}/ks_atendimentocrm-worker:stable
SCHEDULER_IMAGE=${NS}/ks_atendimentocrm-scheduler:stable")"
check "canal móvel EXPLÍCITO → não toca (é decisão de quem opera)" test -z "$R"
check "  o :stable escolhido continua lá, intacto" \
  grep -q "^WORKER_IMAGE=${NS}/ks_atendimentocrm-worker:stable$" "$PIN_DIR/.env"
...
R="$( printf "APP_IMAGE=${NS}/ks_atendimentocrm:1.3.0\n" > "$PIN_DIR/.env"
